...
export default ({ pool, userPoolsData, onFinish }) => {
	...
	useEffect(() => {
		const getData = async () => {
			...
			const { contract_data, domainId, adopted, local } = {
				...pool_data,
			};
			...
			if (typeof amount === "string") {
				if (utils.parseUnits(amount || "0", 18).toBigInt() <= 0) {
					...
					try {
						...
						let amounts =
							await sdk.sdkPool.calculateRemoveSwapLiquidity(
								domainId,
								contract_address,
								_amount
							);
						...
						if (amounts?.length > 1) {
							...
							const tokenIndex =
								await sdk.sdkPool.getPoolTokenIndex(
									domainId,
									contract_address,
									contract_address
								);
					...
					if (!failed) {
						try {
							const approve_request =
								await sdk.sdkBase.approveIfNeeded(
									domainId,
					...
					if (!failed) {
						try {
							...
							if (add_request) {
								...
								if (gasLimit) {
									gasLimit = FixedNumber.fromString(
										gasLimit.toString()
									)
										.mulUnsafe(
								...
								const add_response =
									await signer.sendTransaction(add_request);
					...
					if (!failed) {
						try {
							...
							const remove_request =
								await sdk.sdkPool.removeLiquidity(
									domainId,
									contract_address,
									_amount,
									minAmounts,
	...
	const calculateAddLiquidityPriceImpact = async (
		...
		try {
			...
			if (
				...
				const price_impact =
					await sdk.sdkPool.calculateAddLiquidityPriceImpact(
						domainId,
						contractAddress,
						amountX,
						amountY
				...
				setPriceImpactAdd(
					Number(utils.formatUnits(BigInt(price_impact || "0"), 18)) *
						100
				);
			} else {
				manual = true;
	...
	const x_balance_amount =
		x_asset_data &&
		getBalance(chain_id, x_asset_data.contract_address, balances_data)
			?.amount;
	...
	const y_balance_amount =
		y_asset_data &&
		getBalance(chain_id, y_asset_data.contract_address, balances_data)
			?.amount;
														...
														<span
															className={`${
																disabled
																	...
																	: "cursor-pointer text-blue-400 hover:text-blue-500 dark:text-blue-500 dark:hover:text-blue-400"
															} text-xs font-medium`}
														>
															Max
														</span>
									...
									<button
										...
										className={`w-full ${
											disabled || !valid_amount
												...
												: "bg-blue-500 hover:bg-blue-600 dark:bg-blue-600 dark:hover:bg-blue-700 cursor-pointer text-white"
										} rounded text-base text-center py-3 px-2 sm:px-3`}
									>
										<span className="flex items-center justify-center space-x-1.5">
											{(calling || approving) && (
												<TailSpin
